...
void learningScene1(int width, int height, std::string& filename) {
    ...
    // Define a face model
    //Model face("../models/face.off");
    //Material faceMaterial(Vec3<float>(0.25, 0.45, 0.45), 1.0f, 0.40f);
    //face.scale(Vec3<float>(1.5f, 1.5f, 1.5f));
    //face.translate(Vec3<float>(0.f, -0.8f, -3.3f));
    //face.setMaterial(faceMaterial);
    //// Add face to scene
    //scene.add(face);
    ...
    // Define an area light
    //Vec3<float> lightPos = Vec3<float>(0.f, 1.5f, -4.5f);
    //Vec3<float> lightColor = Vec3<float>(1.f, 1.f, 0.93f);
    //float lightIntensity = 1.0f;
    //Vec3<float> lightDir = Vec3<float>(0.f, -1.f, 0.f);
    //AreaLight areaLight(lightPos, lightColor, lightIntensity, lightDir, 1.8f);
    //scene.add(areaLight);
    ...
    //rayTracer.enablePathTracing(4, 25);
    //rayTracer.enablePathTracing(3, 64);
...
int main(int argc, char *argv[]) {
    ...
    learningScene1(width, height, filename);
